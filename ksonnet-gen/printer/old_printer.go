...
func (p *old_printer) writeByte(ch byte, n int) {
	if p.err != nil {
		return
	}
...
func (p *old_printer) writeString(s string) {
	for _, b := range []byte(s) {
		p.writeByte(b, 1)
	}
}

func (p *old_printer) writeStringNoIndent(s string) {
	for _, b := range []byte(s) {
...
// old_printer prints a node.
// nolint: gocyclo
func (p *old_printer) print(n interface{}) {
	if p.err != nil {
		return
	}
...
func (p *old_printer) handleApply(a *ast.Apply) {
...
func (p *old_printer) handleArguments(a ast.Arguments) {
	argCount := 0
...
func (p *old_printer) handleConditional(c *ast.Conditional) {
	p.writeString("if ")
	p.print(c.Cond)
...
func (p *old_printer) writeComment(c *astext.Comment) {
	if c == nil {
		return
	}
...
func (p *old_printer) handleIndex(i *ast.Index) {
	if i == nil {
...
func (p *old_printer) handleLocal(l *ast.Local) {
	p.writeString("local ")
...
func (p *old_printer) handleLocalFunction(f *ast.Function) {
...
func (p *old_printer) fieldID(kind ast.ObjectFieldKind, expr1 ast.Node, id *ast.Identifier) {
	if expr1 != nil {
		switch t := expr1.(type) {
		default:
			p.print(t)
...
func (p *old_printer) handleObjectComp(oc *ast.ObjectComp) {
	p.writeString("{")
	p.indentLevel++
	p.writeByte(newline, 1)
...
func (p *old_printer) handleArrayComp(ac *ast.ArrayComp) {
	p.writeString("[")
	p.indentLevel++
	p.writeByte(newline, 1)
	p.print(ac.Body)
...
func (p *old_printer) forSpec(spec ast.ForSpec) {
	if spec.Outer != nil {
...
func (p *old_printer) handleObjectField(n interface{}) {
...
func (p *old_printer) addMethodSignature(fun *ast.Function) {
	if fun == nil {
		return
	}
...
func (p *old_printer) indexID(i *ast.Index) {
	if i == nil {
		p.err = errors.New("index is nil")
		return
	}
...
func (p *old_printer) isObjectSingleLine(i interface{}) bool {
	if p.inFunction {
		return true
	}
